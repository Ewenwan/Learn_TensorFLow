...
def model(X, w_h, w_h2, w_o, p_keep_input, p_keep_hidden): 
    ...
    ## 第一层 ##
    h = tf.nn.relu(tf.matmul(X, w_h))   # 非线性激活函数 nonlinear activate function  relu / sigmoid  / maxout
    h = tf.nn.dropout(h, p_keep_hidden) # 第一层 dropout  激活部分神经元
    ## 第二层 ##
    h2 = tf.nn.relu(tf.matmul(h, w_h2)) # 非线性激活函数
    h2 = tf.nn.dropout(h2, p_keep_hidden)# 第二层dropout  激活部分神经元

    return tf.matmul(h2, w_o) # 外加输出层
...
## 模型 
pred_y_x = model(X, w_h, w_h2, w_o, p_keep_input, p_keep_hidden)
